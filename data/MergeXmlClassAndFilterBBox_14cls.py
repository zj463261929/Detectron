...
merge_class_name = [["suv","minitruckbox"], ["bulldozer","roadroller","forklift","grader"], ["digger","pilingmachine"],["car"], ["bus"], ["mixer", "tanker"], ["person"],["minitruck"], ["minibus"],  ["boxtruckbig", "autotruck"], ["boxtrucksmall", "smalltruck"], ["tricycle"], ["bicycle", "motorcycle"]]

merge_class_name_result = ["suv", "forklift", "digger", "car", "bus", "tanker", "person", "minitruck", "minibus", "truckbig", "trucksmall", "tricycle", "bicycle"] #'car'

#move_class_name =["blender", "tower", "brickspile", "sandpile", "cylinderpile", "bagpile", "boardpile","nonamecar","ship","smoke","fire","drillingmachine","rammer","bulldozer","pressmachine","forktruck","farmmachine","crane","tractor"]
...
merge_class_name_temp = []
for i in range(len(merge_class_name)):
    for j in range(len(merge_class_name[i])):
        merge_class_name_temp.append(merge_class_name[i][j])

for i in xrange(len(files)): 
    if len(merge_class_name_result)!=len(merge_class_name):
        print ("class input error!")
    ...
    if os.path.exists(orig_img_dir+basename+".jpg") and os.path.exists(orig_xml_dir+basename+".xml"): #只处理该文件夹下面有的文件
        ...
        for i in range(1,len(lst_temp)):
            ...
            name_lst.append(temp) #<name>GDS</name>
            '''if (temp in merge_class_name_temp) or (temp in move_class_name):# or (temp in single_class_name):
        ...
        for i in range(len(name_lst)):
            temp = name_lst[i]
            if temp in merge_class_name_temp:
                name_lst_temp.append(temp)
       ...
       #增强的数据 xml中没有occluded属性  #len(pose_lst_temp)==len(truncated_lst_temp)==len(difficult_lst_temp)==len(occluded_lst_temp)==
        ...
        for i in range(len(name_lst)):
            ...
            if name_temp in merge_class_name_temp: #
                for ii in range(len(merge_class_name)):
                    if name_temp in merge_class_name[ii]:
                        temp = merge_class_name_result[ii]
            else:
                continue
