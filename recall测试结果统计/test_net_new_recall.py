...
def test_result(logPath,classNum,fw):
	...
	with open(logPath, 'r') as ann_file:
		...
		for ll in lines:
			...
			#存放数据集名称
			if l.startswith("INFO test_engine.py: 321: Wrote detections to:"):
				lst = l.strip().split(":")
				l2 = lst[-1]
				lst2 = l2.strip().split("/")
				l3 = lst2[-3]
			...
			#recall
			if l.startswith("INFO cocoeval.py: 461:  Average Recall     (AR) @[ IoU=0.50      | area=   all | maxDets=100 ] = "):
				lst = l.strip().split(" = ")
				ll = lst[-1]
				recallLst.append(ll)
				
			#AP
			if l.startswith("INFO cocoeval.py: 461:  Average Precision  (AP) @[ IoU=0.50:0.95 | area=   all | maxDets=100 ] = "):
				lst = l.strip().split(" = ")
				ll = lst[-1]
				APLst.append(ll)
			
			#ap50
			if l.startswith("INFO cocoeval.py: 461:  Average Precision  (AP) @[ IoU=0.50      | area=   all | maxDets=100 ] = "):
				lst = l.strip().split(" = ")
				ll = lst[-1]
				AP50Lst.append(ll)
				
			#AP75
			if l.startswith("INFO cocoeval.py: 461:  Average Precision  (AP) @[ IoU=0.75      | area=   all | maxDets=100 ] = "):
				lst = l.strip().split(" = ")
				ll = lst[-1]
				AP75Lst.append(ll)
				
			#APs
			if l.startswith("INFO cocoeval.py: 461:  Average Precision  (AP) @[ IoU=0.50:0.95 | area= small | maxDets=100 ] = "):
				lst = l.strip().split(" = ")
				ll = lst[-1]
				APsLst.append(ll)
				
			#APm
			if l.startswith("INFO cocoeval.py: 461:  Average Precision  (AP) @[ IoU=0.50:0.95 | area=medium | maxDets=100 ] = "):
				lst = l.strip().split(" = ")
				ll = lst[-1]
				APmLst.append(ll)
				
			#APl
			if l.startswith("INFO cocoeval.py: 461:  Average Precision  (AP) @[ IoU=0.50:0.95 | area= large | maxDets=100 ] = "):
				lst = l.strip().split(" = ")
				ll = lst[-1]
				APlLst.append(ll)
