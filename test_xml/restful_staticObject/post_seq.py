...
def singlePredict(mm,imgPath, output_xmlpath,num_no_target, maxArea):
    ...
    if not os.path.exists(imgPath):
        ...
        return None,cls_lst,score_lst,xmin_lst,ymin_lst,xmax_lst,ymax_lst,num_no_target
    ...
    if len(datalist) < 1:
        ...
        #continue
        return img_np,cls_lst,score_lst,xmin_lst,ymin_lst,xmax_lst,ymax_lst,num_no_target
...
def drawResult(img, cls_lst,score_lst,xmin_lst,ymin_lst,xmax_lst,ymax_lst,color=0,pixel=2):
    for i in range(len(score_lst)):
        ...
        ymax = ymax_lst[i]
        strname = cls_lst[i]
        strscore = round(float(score_lst[i]),2)
        ...
        if 0==color:
            ...
            cv2.putText(img, strname+str(strscore) + '('+str(xmin)+','+ str(ymin)+','+str(xmax)+','+str(ymax)+')', (xmin,ymin), font, 1,(0,0,255),2)
        else:
            ...
            cv2.putText(img, strname+str(strscore) + '('+str(xmin)+','+ str(ymin)+','+str(xmax)+','+str(ymax)+')', (xmin,ymin), font, 1,(0,255,0),2)
...
def filterBBox(img_all,cls_lst_all,score_lst_all,xmin_lst_all,ymin_lst_all,xmax_lst_all,ymax_lst_all, score_th,iou_th,th):
    x1_lst = []
    y1_lst = []
    x2_lst = []
    y2_lst = []
    ...
    for i in range(len(score_lst_all)): #image num ####################
        ...
        for j in range(len(score_lst_all[i])): #bbox num
            ...
            static_lst.append(mv_arr_index[n])
            n = n + 1
    ...
    return static_lst_res #c_lst_res,s_lst_res,static_lst_res,x1_lst_res,y1_lst_res,x2_lst_res,y2_lst_res          
            ...
            if file.endswith('.jpg'):
                ...
                for file_new in files_new:
                    if file_new.endswith('.jpg'):
                        ...
                        img,cls_lst,score_lst,xmin_lst,ymin_lst,xmax_lst,ymax_lst,num_no_target = singlePredict(mm,imgPath, xmlPath,num_no_target, maxArea)
                ...
                if PRINT:
                    for ii in range(len(img_all)):
                        ...
                        for jj in range(len(xmin_lst_all[ii])):
                            print xmin_lst_all[ii][jj],ymin_lst_all[ii][jj],xmax_lst_all[ii][jj],ymax_lst_all[ii][jj],cls_lst_all[ii][jj]
                            
                start = time.time()
                s = requests       
                data_lst = []
                for img_index in range(len(imgPath_all)):
                    singleImgInfo_lst = {"imgPath":imgPath_all[img_index],"cls":cls_lst_all[img_index],"score":score_lst_all[img_index],"xmin":xmin_lst_all[img_index],"ymin":ymin_lst_all[img_index],"xmax":xmax_lst_all[img_index],"ymax":ymax_lst_all[img_index]}

                    data_lst.append(singleImgInfo_lst)   
                #data = {"data":data_lst}  
                data = {"data":data_lst,"param":{"scoreth":1.0,"aveth":35,"iouth":0.7}}  
                #print data_lst
                print "11111: ", data
                ...
                xmax_lst_all_res1 = []
                ymax_lst_all_res1 = []
                for img_index in range(len(datalist)):
                    singleImgInfo_lst = datalist[img_index]
                    keys_lst = singleImgInfo_lst.keys()
                    for i in range( len(keys_lst) ):
                        if keys_lst[i]=="imgPath":
                            imgPath_all_res1.append( singleImgInfo_lst["imgPath"] )
                        if keys_lst[i]=="cls":
                            cls_lst_all_res1.append( singleImgInfo_lst["cls"] )
                        if keys_lst[i]=="score":
                            score_lst_all_res1.append( singleImgInfo_lst["score"] )
                        if keys_lst[i]=="statics":
                            static_lst_all_res.append( singleImgInfo_lst["static"] )
                        if keys_lst[i]=="xmin":
                            xmin_lst_all_res1.append( singleImgInfo_lst["xmin"] )
                        if keys_lst[i]=="ymin":
                            ymin_lst_all_res1.append( singleImgInfo_lst["ymin"] )
                        if keys_lst[i]=="xmax":
                            xmax_lst_all_res1.append( singleImgInfo_lst["xmax"] )
                        if keys_lst[i]=="ymax":
                            ymax_lst_all_res1.append( singleImgInfo_lst["ymax"] )

                if PRINT:
                    print "orig xy:"
                    for ii in range(len(xmin_lst_all)):
                        for jj in range( len(xmin_lst_all[ii]) ):
                            print ii, xmin_lst_all[ii][jj],ymin_lst_all[ii][jj],xmax_lst_all[ii][jj],ymax_lst_all[ii][jj],cls_lst_all[ii][jj]
                    
                    print "res xy:"
                    for ii in range(len(xmin_lst_all_res)):
                        for jj in range( len(xmin_lst_all_res[ii]) ):
                            print ii, xmin_lst_all_res[ii][jj],ymin_lst_all_res[ii][jj],xmax_lst_all_res[ii][jj],ymax_lst_all_res[ii][jj],cls_lst_all_res[ii][jj]
                   #static_lst_res=filterBBox(img_all,cls_lst_all,score_lst_all,xmin_lst_all,ymin_lst_all,xmax_lst_all,ymax_lst_all, 1.0,0.7,35)
                ...
                xmax_lst_all_res = []
                ymax_lst_all_res = []
                for ii in range(len(static_lst_all_res)):
                    cls_lst_temp = []
                    score_lst_temp = []
                    xmin_lst_temp = []
                    ymin_lst_temp = []
                    xmax_lst_temp = []
                    ymax_lst_temp = []
                    for jj in range(len(static_lst_all_res[ii])):
                        if 0==static_lst_all_res[ii][jj]:
                            cls_lst_temp.append(cls_lst_all_res1[ii][jj])
                            score_lst_temp.append(score_lst_all_res1[ii][jj])
                            xmin_lst_temp.append(xmin_lst_all_res1[ii][jj])
                            ymin_lst_temp.append(ymin_lst_all_res1[ii][jj])
                            xmax_lst_temp.append(xmax_lst_all_res1[ii][jj])
                            ymax_lst_temp.append(ymax_lst_all_res1[ii][jj])
                    cls_lst_all_res.append(cls_lst_temp)
                    score_lst_all_res.append(score_lst_temp)
                    xmin_lst_all_res.append(xmin_lst_temp)
                    ymin_lst_all_res.append(ymin_lst_temp)
                    xmax_lst_all_res.append(xmax_lst_temp)
                    ymax_lst_all_res.append(ymax_lst_temp)
                
                for ii in range(len(imgPath_all_res1)): #画检测结果                   
                    img_temp = cv2.imread(imgPath_all_res1[ii],1)
                    cv2.putText(img_temp, str(file) + "_" + str(ii)+"/" + str(len(img_all)) , (30,30), font, 1,(0,0,255),2)
                    
                    img_temp = drawResult(img_temp, cls_lst_all[ii],score_lst_all[ii],xmin_lst_all[ii],ymin_lst_all[ii],xmax_lst_all[ii],ymax_lst_all[ii],color=0)#画原始检测结果
                    
                    img_temp = drawResult(img_temp, cls_lst_all_res[ii],score_lst_all_res[ii],xmin_lst_all_res[ii],ymin_lst_all_res[ii],xmax_lst_all_res[ii],ymax_lst_all_res[ii],color=1) #画校正后的结果
                    
                    file_new = os.path.basename(imgPath_all_res1[ii])
                    savePath = os.path.join(output_image,file_new) 

                    cv2.imwrite(savePath, img_temp)
...
if __name__ == '__main__':
    ...
    cfg_path = '/opt/yangguan/zhangjing/Detectron/test_xml/model/retinanet_79_129999.yaml' #retinanet_71.yaml' 
    weights_path = '/opt/yangguan/zhangjing/Detectron/test_xml/model/model_79_129999.pkl'  #'
    input_imagespath = '/opt/yangguan/zhangjing/Detectron/test_xml/test2'
    replot_input_imagespath = '/opt/yangguan/zhangjing/Detectron/test_xml/test2' #img_replot
    output_image     = '/opt/yangguan/zhangjing/Detectron/test_xml/out222'
    output_xmlpath   = '/opt/yangguan/zhangjing/Detectron/test_xml/xml222'
    output_orig_image     = '/opt/yangguan/zhangjing/Detectron/test_xml/out_orig'
